# Simple ML microservice with rule-based predictions for immediate use
...
def recommend_best(payload: dict):
    # return scoring for provided candidates
    ...
    for c in candidates:
        ...
        sust = 60
        if c.get('unit_cost'):
            cost = float(c.get('unit_cost'))
        else:
            cost = 2.0
        final = perf_avg*0.6 + (sust/100)*0.4 - (cost/10)
        results.append({'fabric': c.get('name','unknown'), 'score': round(final,3), 'perf_avg': round(perf_avg,3), 'sustainability': sust, 'unit_cost': cost})
